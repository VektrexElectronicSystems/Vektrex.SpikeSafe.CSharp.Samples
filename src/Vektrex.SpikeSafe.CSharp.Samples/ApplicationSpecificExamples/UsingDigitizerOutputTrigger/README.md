...
## Overview 
The SpikeSafe's integrated Digitizer has a Send External Instrument Trigger Out signal that is controlled through SCPI. The SpikeSafe PRF and PSMU both have an Input Trigger that can be configured to start certain modes' output in response to a hardware trigger, either from the Digitizer or an external instrument.
...
- **On Time:** 1s
- **Off Time:** 1s
- **Input Trigger Source:** External (necessary to respond to Digitizer Send External Instrument Trigger Out signal)
